...
class CheckIndexHealthMFASyslog extends Command
{
    ...
    public function handle()
    {
        ...
        $log = new Logger('index_health_check_mfa_syslog');
        $log->pushHandler(new StreamHandler(storage_path('logs/checks/'.$today.'_index_health_check_mfa_syslog.log'), Logger::INFO));

        $log->info('Starting MFA syslog Index Health Check!');
        ...
        $log->info('mfa_syslog index health check command completed!');
    }
