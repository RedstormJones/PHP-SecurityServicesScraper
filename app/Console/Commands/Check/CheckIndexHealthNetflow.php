...
class CheckIndexHealthNetflow extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'check:netflow';
     ...
     * @var string
     */
    protected $description = 'Check index health for netflow';
    ...
    public function handle()
    {
        ...
        $log = new Logger('index_health_check_netflow');
        $log->pushHandler(new StreamHandler(storage_path('logs/checks/'.$today.'_index_health_check_netflow.log'), Logger::INFO));

        $log->info('Starting netflow Index Health Check!');
        ...
        file_put_contents(storage_path('app/responses/indexcheck-netflow.json'), $json_response);
        ...
        $log->info('netflow index health check command completed!');
    }
