...
class CheckIndexHealthElastAlert extends Command
{
    ...
    {
        Log::info(PHP_EOL.'[CheckIndexHealthElastAlert.php] Starting ElastAlert Index Health Check!'.PHP_EOL);

        // setup date and threshold variables
        $date = Carbon::now()->toDateString();
        $threshold_timestamp = Carbon::now()->subMinutes(5);
        Log::info('[CheckIndexHealthElastAlert.php] threshold timestamp: '.$threshold_timestamp);
        ...
        Log::info('[CheckIndexHealthElastAlert.php] elastic url: '.$es_url);
        ...
        try {
            ...
            // pop smoke and bail
            Log::error('[!] [CheckIndexHealthElastAlert.php] failed to decode JSON response: '.$e->getMessage());
            die('[!] [CheckIndexHealthElastAlert.php] failed to decode JSON response: '.$e->getMessage().PHP_EOL);
        }
        ...
        if (array_key_exists('hits', $response)) {
            ...
            Log::info('[CheckIndexHealthElastAlert.php] last log timestamp: '.$last_log_timestamp);
            ...
            Log::info('[CheckIndexHealthElastAlert.php] carbon last log timestamp: '.$last_log_timestamp);
            ...
            // build error string
            $error_string = '[!] [CheckIndexHealthElastAlert.php] '.$error['type'].' - '.$error['index'].PHP_EOL.'reason: '.$error['reason'];
            ...
            Log::error('[!] [CheckIndexHealthElastAlert.php] no hits found for search query..');
            die('[!] [CheckIndexHealthElastAlert.php] no hits found for search query..'.PHP_EOL);
        }

        Log::info('[CheckIndexHealthElastAlert.php] ElastAlert index health check command completed!'.PHP_EOL);
    }
    ...
    public function logToMSTeams($message)
    {
        ...
        Log::info('[CheckIndexHealthElastAlert.php] MS Teams post data: '.$json_post_data);
        ...
        if ($response == 1) {
            Log::info('[CheckIndexHealthElastAlert.php] post to Teams channel successful!');
        } else {
            Log::error('[!] [CheckIndexHealthElastAlert.php] post to Teams channel failed!');
            die('[!] [CheckIndexHealthElastAlert.php] post to Teams channel failed!'.PHP_EOL);
        }
    }
}
