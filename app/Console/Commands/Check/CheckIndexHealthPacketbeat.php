...
class CheckIndexHealthPacketbeat extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'check:packetbeat';
     ...
     * @var string
     */
    protected $description = 'Check index health for packetbeat';
    ...
    {
        Log::info(PHP_EOL.'[CheckIndexHealthPacketbeat.php] Starting packetbeat Index Health Check!'.PHP_EOL);

        // setup date and threshold variables
        $date = Carbon::now()->toDateString();
        $threshold_timestamp = Carbon::now()->subMinutes(5);
        Log::info('[CheckIndexHealthPacketbeat.php] threshold timestamp: '.$threshold_timestamp);
        ...
        Log::info('[CheckIndexHealthPacketbeat.php] elastic url: '.$es_url);
        ...
        try {
            ...
            // pop smoke and bail
            Log::error('[!] [CheckIndexHealthPacketbeat.php] failed to decode JSON response: '.$e->getMessage());
            die('[!] [CheckIndexHealthPacketbeat.php] failed to decode JSON response: '.$e->getMessage().PHP_EOL);
        }
        ...
        if (array_key_exists('hits', $response)) {
            ...
            Log::info('[CheckIndexHealthPacketbeat.php] last log timestamp: '.$last_log_timestamp);
            ...
            Log::info('[CheckIndexHealthPacketbeat.php] carbon last log timestamp: '.$last_log_timestamp);
            ...
            if ($last_log_timestamp->lessThanOrEqualTo($threshold_timestamp)) {
                ...
                // we're good
                Log::info('[CheckIndexHealthPacketbeat.php] '.$index.' within acceptable range');
            }
            ...
            // build error string
            $error_string = '[!] [CheckIndexHealthPacketbeat.php] '.$error['type'].' - '.$error['index'].PHP_EOL.'reason: '.$error['reason'];
            ...
            Log::error('[!] [CheckIndexHealthPacketbeat.php] no hits found for search query..');
            die('[!] [CheckIndexHealthPacketbeat.php] no hits found for search query..'.PHP_EOL);
        }

        Log::info('[CheckIndexHealthPacketbeat.php] packetbeat index health check command completed!'.PHP_EOL);
    }
    ...
    public function logToSlack($message)
    {
        ...
        Log::info('[CheckIndexHealthPacketbeat.php] slack post data: '.$json_post_data);
        ...
        if ($response == 'ok') {
            Log::info('[CheckIndexHealthPacketbeat.php] post to slack channel succeeded!');
        } else {
            Log::error('[!] [CheckIndexHealthPacketbeat.php] post to slack channel failed!');
            die('[!] [CheckIndexHealthPacketbeat.php] post to slack channel failed!'.PHP_EOL);
        }
    }
    ...
    public function logToMSTeams($message)
    {
        ...
        if ($response == 1) {
            Log::info('[CheckIndexHealthPacketbeat.php] post to Teams channel successful!');
        } else {
            Log::error('[!] [CheckIndexHealthPacketbeat.php] post to Teams channel failed!');
            die('[!] [CheckIndexHealthPacketbeat.php] post to Teams channel failed!'.PHP_EOL);
        }
    }
}
