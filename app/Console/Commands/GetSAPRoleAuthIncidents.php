...
class GetSAPRoleAuthIncidents extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'get:saproleauthincidents';
     ...
     * @var string
     */
    protected $description = 'Get new SAP role authorization incidents';
    ...
    {
        /*
         * [1] Get all SAP role auth incidents email
         */

        Log::info(PHP_EOL.PHP_EOL.'*********************************************'.PHP_EOL.'* Starting SAP role auth incidents crawler! *'.PHP_EOL.'*********************************************');
        ...
        $url = 'https:/'.'/kiewit.service-now.com/api/now/v1/table/incident?sysparm_display_value=true&assignment_group=KTG%20-%20SAP%20Role%20Authorization';
        ...
        file_put_contents(storage_path('app/responses/saprole_incidents.dump'), $response);

        // JSON decode response
        $saprole_incidents = \Metaclassing\Utility::decodeJson($response);

        // grab the data we care about and log how many incidents we have
        $incidents = $saprole_incidents['result'];
        Log::info('total SAP role auth incident count: '.count($incidents));

        // JSON encode and dump incident collection to file
        file_put_contents(storage_path('app/collections/saprole_incidents_collection.json'), \Metaclassing\Utility::encodeJson($incidents));

        /*
         * [2] Process SAP role auth incidents into database
         */

        Log::info(PHP_EOL.'************************************************'.PHP_EOL.'* Starting SAP role auth incidents processing! *'.PHP_EOL.'************************************************');

        // cycle through security incidents and add the new ones
        foreach ($incidents as $incident) {
            // try to find existing record with matching sys_id
            $exists = ServiceNowSapRoleAuthIncident::where('sys_id', $incident['sys_id'])->value('id');
            ...
            if ($exists) {
                ...
                $incidentmodel = ServiceNowSapRoleAuthIncident::find($exists);
                ...
                Log::info('updated SAP role auth incident: '.$incident['number']);
            } else {
                // otherwise, create a new security incident record
                Log::info('creating new SAP role auth incident: '.$incident['number']);
                ...
                */
                ...
                $new_incident = new ServiceNowSapRoleAuthIncident();
        ...
        Log::info('* Completed SAP role authorization incidents! *');
    }
