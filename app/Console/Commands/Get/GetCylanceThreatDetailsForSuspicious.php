...
class GetCylanceThreatDetailsForSuspicious extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'get:suspiciousthreatdetails';
     ...
     * @var string
     */
    protected $description = 'Get details of suspicious Cylance threats.';
    ...
    public function handle()
    {
        ...
        Log::info(PHP_EOL.PHP_EOL.'*******************************************************'.PHP_EOL.'* Starting Cylance suspicious threat details crawler! *'.PHP_EOL.'*******************************************************');
        ...
        $suspicious_threat_details = [];

        // pluck a collection of threat id's from the securitymetrics db
        $threat_ids = CylanceThreat::where('suspicious_in_devices', '>', 0)->pluck('threat_id');
        ...
        foreach ($threat_ids as $threat_id) {
            Log::info('querying suspicious threats for threat id: '.$threat_id);

            // setup url
            $url = 'https:/'.'/protect.cylance.com/ThreatDetails/DevicesSuspicious?filehashId='.$threat_id;
            ...
            do {
                ...
                file_put_contents(storage_path('app/responses/suspicious_threat_details.response'), $response);
            ...
            $suspicious_threat_details[] = $threat_collection;
        }

        // simple array to hold all threat detail objects collected in a simple array
        $suspicious_threats_collection = [];

        foreach ($suspicious_threat_details as $suspicious_threats) {
            foreach ($suspicious_threats as $threat) {
                $suspicious_threats_collection[] = $threat;
            }
        }

        // log count
        Log::info('total suspicious threat collection count: '.count($suspicious_threats_collection));

        $device_threat_details = [];

        Log::info('converting millisecond timestamps to datetimes...');
        foreach ($suspicious_threats_collection as $data) {
        ...
        foreach ($device_threat_details as $device_threat) {
            ...
            $url = 'http://10.243.32.36:9200/cylance_threat_details_suspicious/cylance_threat_details_suspicious/'.$es_id.'/_update';
        ...
        Log::info('Suspicious threat details execution time: '.$execution_hours.':'.$execution_mins.':'.$execution_secs);

        Log::info('* Cylance suspicious threat details completed! *');
    }
