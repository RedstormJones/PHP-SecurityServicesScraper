...
class GetCylanceThreatDetailsForBlocked extends Command
{
    ...
    public function handle()
    {
        ...
        $threat_ids = CylanceThreat::where('blocked_in_devices', '>=', 10)->pluck('threat_id');
        ...
        foreach ($threat_ids as $threat_id) {
            ...
            $blocked_threat_details[] = $threat_collection;
        }
        ...
        foreach ($device_threat_details as $data) {
            ...
            $contents = file_get_contents(storage_path('app/collections/blocked_threat_details.json'));

            // append JSON data string to current active threat contents and dump back to file
            file_put_contents(storage_path('app/collections/blocked_threat_details.json'), $contents.$json_data);
        }
