...
class GetCASAlertsHigh extends Command
{
    ...
    {
        Log::info('[GetCASAlertsHigh.php] Starting MCAS High Alerts API Poll!');
        ...
        Log::info('[GetCASAlertsHigh.php] current timestamp: '.$current_timestamp);
        Log::info('[GetCASAlertsHigh.php] alert datetime threshold (ms): '.$alert_threshold_ms);
        ...
        Log::info('[GetCASAlertsHigh.php] sending '.count($alerts).' CAS high alerts to Kafka...');
        ...
        foreach ($alerts as $alert) {
            ...
            if ($result[0]['data'][0]['partitions'][0]['errorCode']) {
                Log::error('[GetCASAlertsHigh.php] Error sending CAS high alert to Kafka: '.$result[0]['data'][0]['partitions'][0]['errorCode']);
            } else {
