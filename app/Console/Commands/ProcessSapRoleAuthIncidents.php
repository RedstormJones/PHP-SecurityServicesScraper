...
class ProcessSapRoleAuthIncidents extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'process:saproleauthincidents';
     ...
     * @var string
     */
    protected $description = 'Get new SAP Role Auth incident data and update the database';
    ...
    public function handle()
    {
        ...
        $contents = file_get_contents(storage_path('app/collections/saprole_incidents_collection.json'));

        // JSON decode collection
        $saprole_incidents = \Metaclassing\Utility::decodeJson($contents);

        // cycle through security incidents and add the new ones
        foreach ($saprole_incidents as $incident) {
...
}   // end of ProcessSapRoleAuthIncidents command class
